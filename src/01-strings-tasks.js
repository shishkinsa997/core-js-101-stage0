 ...
 */
function getStringFromTemplate(f, s) {
  return `Hello, ${f} ${s}!`;
}
...
function extractNameFromTemplate(v) {
  return v.replace(/Hello, /, '').replace(/!/, '');
}
...
function getRectangleString(w, h) {
  ...
  return `${top}${h > 2 ? mid.repeat(h - 2) : ''}${bot}`;
}
