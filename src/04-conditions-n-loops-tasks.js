...
function doRectanglesOverlap(rect1, rect2) {
  const isHeight1 = rect1.top + rect1.height <= rect2.top;
  const isWidth1 = rect1.left + rect1.width <= rect2.left;

  return !(isHeight1 || isWidth1);
}
...
function findFirstSingleChar(str) {
  for (let j = 0; j < str.length; j += 1) {
    // Проверяем ВСЕ вхождения символа: до и после текущей позиции
    if (str.indexOf(str[j]) === j && str.indexOf(str[j], j + 1) === -1) {
      return str[j];
    }
  }
  return null;
